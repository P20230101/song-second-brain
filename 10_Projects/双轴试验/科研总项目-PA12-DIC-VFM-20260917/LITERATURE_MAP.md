...
更新时间：2026-09-18。下表是研究路线审查用的证据矩阵，不是最终投稿参考文献表。正式写作前仍需逐篇核对全文、版本、页码和 DOI。
...
- [Xiao et al. 2016, cruciform specimen simulation optimization](https://www.atlantis-press.com/proceedings/mmeceb-15/25847210)
- [Mazzoleni et al. 2015, Gaussian pre-filtering for DIC uncertainty](https://doi.org/10.1016/j.optlaseng.2014.08.004)
- [Vitucci 2023, cruciform specimens and constitutive identification, arXiv:2308.01260](https://arxiv.org/abs/2308.01260)
...
## 6. 89 条记录的字段审计结论（2026-09-20）

逐条字段盘点与 ID 清单见 [LITERATURE_DATABASE｜字段状态审计](LITERATURE_DATABASE.md#e-字段状态审计-2026-09-18)。本轮已将 89/89 条记录映射到 `已核验/摘要核验/仅线索/缺失`：16/38/34/1。这里的“已核验”只表示正文、图注或图表被实际读取，不表示 89 条都已完成全文审计；BX19 的已核验状态仍是 arXiv 预印本，不等于同行评审。当前 12 条已完成项目本地 PDF/MinerU 或 pypdf 绑定：P00、PA19A、PA22–PA25、PA28、BX16、BX18、BX19、VF25、DIC01；PA22–PA25、PA28 的正文/图像还来自官方开放全文核验；VF08 已完成官方在线全文/图像核验，但本地 PDF/MinerU 待补。

当前最实质的缺口不是候选数量，而是证据粒度：主要创新/局限只在核心矩阵 15 条中规范化；42 条数据库记录没有直接外部来源链接；VF18 的原登记 DOI 未通过 Crossref/OpenAlex 元数据核验，BX19 是未经过同行评审的 arXiv 预印本。新增 DIC01 和 BX19 分别强化了 DIC 噪声与双轴几何/参数抽取的证据，但不改变 PA12–DIC–VFM 组合缺口的边界。故研究 Gap 仍只能写成“在明确工艺、批次和方向的 SLS PA12 中，比较整体曲线与 DIC 全场 + 边界力在参数可辨识性、误差传播和完全留出路径预测上的差异”，并标注为中等置信度候选缺口，不能写成 87 篇逐篇全文审计后确认的国际空白。

该审计不改变已排除的宽泛表述（首次 PA12 DIC/VFM、首次 PA12 双轴、首次 VFM 可辨识性）；它只收紧了证据边界，并将 VF18 从摘要核验降为待补证。

## 7. 定向检索复核（2026-09-19）

本轮按四组组合词检索：`SLS PA12 + DIC + VFM`、`PA12 + biaxial cruciform + constitutive identification`、`VFM + parameter identifiability + biaxial`、`SLS PA12 + rate-dependent/anisotropic/damage`。以下来源均回到出版社、期刊或开放全文页面核对；它们用于收紧研究边界，不计作“PA12 双轴 VFM”先例：

| 来源 | 直接核实到的事实 | 对本项目的影响 |
|---|---|---|
| Marek, Davis & Pierron 2017, *Sensitivity-based virtual fields for the non-linear virtual fields method*, DOI [10.1007/s00466-017-1411-6](https://doi.org/10.1007/s00466-017-1411-6)，开放全文入口 [PMC](https://pmc.ncbi.nlm.nih.gov/articles/PMC6961464/) | SBVF 以参数应力敏感度构造随时间变化的虚场；论文比较了线性/Voce 塑性和噪声情景，并明确参数激活区域与时间并不相同。 | “有全场”不能等同于“所有参数可辨识”；Gate 7 必须报告灵敏度秩、条件数和参数相关性。 |
| Marek et al. 2019, *Experimental Validation of the Sensitivity-Based Virtual Fields for Identification of Anisotropic Plasticity Models*, *Experimental Mechanics*, DOI [10.1007/s11340-019-00575-3](https://doi.org/10.1007/s11340-019-00575-3) | 期刊页面报告 Hill48/Yld2000-2D、深缺口试验、立体 DIC 与 SBVF 的实验验证；作者还指出单个异质试验对更复杂模型的全应力空间覆盖仍可能不足。 | M4/M5 不能因为使用 DIC 或 SBVF 就自动开放；必须验证当前加载路径是否覆盖新增参数。 |
| Jones et al. 2018, *Parameter covariance and non-uniqueness in material model calibration using the Virtual Fields Method*, *Computational Materials Science*, DOI [10.1016/j.commatsci.2018.05.037](https://doi.org/10.1016/j.commatsci.2018.05.037) | 研究将全场 VFM 用于率相关模型，并直接讨论参数协方差、非唯一性、参数界限和初值对标定的影响。 | “优化器收敛”不再作为 Gate 5/6 的充分证据；需要 profile/Bootstrap 或等价的不确定性分析。 |
| Zhang et al. 2018, *Accuracy-enhanced constitutive parameter identification using virtual fields method and special stereo-digital image correlation*, *Optics and Lasers in Engineering*, 103, 55–64, DOI [10.1016/j.optlaseng.2017.11.016](https://doi.org/10.1016/j.optlaseng.2017.11.016) | 研究报告 2D-DIC 的离面运动/转动会降低 VFM 识别精度，并以特殊立体 DIC 对比验证误差影响。 | 当前 2D 平面假设必须成为数据准入条件；若无离面运动证据，只能做敏感性情景，不能把 2D 结果写成材料真值。 |

这轮定向检索仍未找到可直接替代当前判断的完整组合先例（同一 SLS PA12 工艺/批次/方向、双轴 DIC 全场、VFM、本构模型判别、参数可辨识性和完全留出路径预测同时成立）。该结果的置信度仍为**中等**：检索加强了反例覆盖，但不能据此写成“国际首次”或“无人做过”。
