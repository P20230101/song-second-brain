...
| 3 Abaqus 真值 | Stage-A 基准通过；真实 PA12 阻塞。基准 ODB 已有独立场量/反力并完成离散 VFM 闭环 |
| 4 无噪声回收 | Stage-A 基准通过；真实 PA12 未开始。三条基准路径回收误差已记录，但材料/几何/实验契约仍是仿真基准 |
| 5 多初值 | 未开始 |
...
1. 核对 `.dat` 字段 schema、单位和有效点掩膜；legacy 与 `.dat` 的首帧交叉检查已完成，后续仍不把两个布局不同的导出强行拼成同一场。
2. 利用已通过的基准 FE–VFM 接口，建立 M0–M2 的独立真值模型、留出路径和模型形式误差试验；先做无噪声回收，再做多初值和噪声。
