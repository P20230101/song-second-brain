...
- `inbox/对话/每日对话记录.md` 是本地单一的原始对话日志，只能在当天日期节下追加，不覆盖历史内容；该文件被 Git 忽略，不发布到公开 Pages。
- `wiki/研究上下文.md` 是每日分析的唯一论题/开题报告依据。没有来源或用户确认的内容不得写入为研究事实。
- 每日整理前先读取本文件、`index.md`、`wiki/研究上下文.md` 和 `inbox/对话/每日对话记录.md` 当天日期节。
- 分析结果必须分开写出来源事实、基于研究上下文的推断、待验证问题和下一步行动；推断不得伪装成事实。
- 对话追加和 Wiki 写回是两个动作：先保留原始记录，再由用户确认可复用结论后更新 `wiki/`、`index.md` 和 `log.md`。
- 没有形成可复用结论时，只更新当天的 `研究报告/日报/YYYY-MM-DD.md`，不创建孤立主题页。
