...
去重规则：同一 DOI 只保留一行；无 DOI 的条目保留作者、题名、期刊和来源位置，并标为待核。不得用书籍、标准、软件手册或坏 DOI 填补“论文数”。当前注册表共 89 个唯一条目，其中 `P00` 是项目内部学位论文、`VF40` 是方法专著、`BX19` 是 arXiv 预印本；其余为论文/方法条目。三者均不计入同行评审论文数。
...
| PA17 | Lammens, De Baere & Van Paepegem 2016, “On the orthotropic elasto-plastic material response of additively manufactured polyamide 12”, [UGent 开放会议全文](https://biblio.ugent.be/publication/8086754) | SLS PA12；三种打印方向单轴拉伸；每个方向 4 个试样；3D DIC | 全弹塑性曲线；方向性与黏性贡献审查 | 是 | 否 | 否 | 是 | 是 | 未重点报告 | 全文+图像 | 公开会议全文；正文给出 P395/PA2200、50/50 新旧粉、2 Hz 相机、各向同性弹性与方向性失效；不计同行评审期刊篇数，不与 PA19/PA19A 合并 |
...
| PA19A | Faes, Wang, Lava & Moens 2015, “Variability in the mechanical properties of laser sintered PA-12 components”, [官方会议 PDF](https://utw10945.utweb.utexas.edu/sites/default/files/2015/2015-69-Faes.pdf) | 审稿会议论文；EOS P395/PA2200；90 个单轴试样；DIC；双轴应变状态验证 | 平面应力正交弹性；VFM 识别 `Qij`/工程常数 | 是 | 是 | 是（双轴几何/加载） | 否 | 是/最终近似各向同性 | 是 | 全文+图像 | 10 页 PDF 已由 pypdf 打开；正文由 MinerU 本地解析，含 11 个 PDF 图像对象；为 PA19 期刊论文的独立会议版本，不能与其合并计数 |
...
| PA28 | Slager, Earp & Ibrahim 2024, “Influence of Build Orientation and Part Thickness on Tensile Properties of Polyamide 12 Parts Manufactured by Selective Laser Sintering”, DOI [10.3390/polym16162241](https://doi.org/10.3390/polym16162241), PMC [全文入口](https://pmc.ncbi.nlm.nih.gov/articles/PMC11360372/) | SLS PA12；144 个 ASTM-D638 试样；0.8/3.2 mm；多打印方向；Video Gauge | 方向/厚度对 E、屈服、UTS、断裂应变和泊松比的统计表征；提示正交模型需按厚度与方向校准 | 否 | 否 | 否 | 否 | 是 | 未报告 | 全文+图像 | 直接约束 XY/XZ 与厚度/批次外推；不是 VFM 或本构参数识别证据；21 页 PDF 已由 pypdf 打开，MinerU 正文/图注已绑定，未导出独立图片文件 |
...
| BX05 | Vitucci 2024, “Biaxial Extension of Cruciform Specimens: Embedding Equilibrium Into Design and Constitutive Characterization”, DOI [10.1007/s11340-024-01052-2](https://doi.org/10.1007/s11340-024-01052-2)，作者机构开放全文 [IRIS PDF](https://iris.poliba.it/handle/11589/267160) | 弹性体/十字形双轴；DIC；等双轴试验 | 平衡约束的本构表征；线弹性参数识别；FE 几何优化 | 是 | 否 | 是 | 未报告 | 否 | 部分 | 全文+图像 | 12 页同行评审全文已由 pypdf/MinerU 绑定；正文明确用 load-cell 合力、DIC 应变线和平衡积分识别弹性参数；不移植其软材料参数 |
...
| BX18 | Xiao et al. 2016, “Design of Biaxial Tensile Cruciform Specimen Based on Simulation Optimization”, DOI [10.2991/mmeceb-15.2016.37](https://doi.org/10.2991/mmeceb-15.2016.37) | 45 钢；十字形双轴；有限元优化 | 中心区应力/应变均匀性与圆角、厚度参数优化 | 未报告 | 否 | 是 | 未报告 | 否 | 部分 | 全文+图像 | 双轴试样几何设计；不移植其金属参数 |
| BX19 | Vitucci 2023, “Cruciform specimens biaxial extension performance relationship to constitutive identification”, [arXiv:2308.01260](https://arxiv.org/abs/2308.01260) | 十字形等双轴；数值形状优化；DIC 实验验证；沿测量线利用全场应变拟合参数 | 试样性能指标与本构参数拟合；非 PA12 专属 | 是 | 否 | 是 | 否 | 未报告 | 部分 | 全文+图像 | 预印本；用于双轴几何、ROI/测量线和参数信息设计，不计同行评审证据 |

## C. DIC、全场应力、VFM/FEMU 与参数可辨识性

| ID | 作者/年份/题名与 DOI | 材料/实验 | 本构/识别方法 | DIC | VFM | 双轴 | 速率 | 各向异性 | 可辨识性 | 正文/图像证据 | 本项目用途 |
|---|---|---|---|---:|---:|---:|---:|---:|---:|---|---|
| DIC01 | Mazzoleni, Matta, Zappa, Sutton & Cigada 2015, “Gaussian pre-filtering for uncertainty minimization in digital image correlation using numerically-designed speckle patterns”, DOI [10.1016/j.optlaseng.2014.08.004](https://doi.org/10.1016/j.optlaseng.2014.08.004) | 数字图像相关；数值设计散斑；图像噪声、子区尺寸和频率内容 | 高斯预滤波与 DIC 测量不确定度 | 是 | 否 | 否 | 否 | 否 | 未报告 | 全文+图像 | 为 Gate 6 建立 DIC 噪声/预滤波敏感性依据；不作为 PA12 本构或 VFM 先例 |
...
| VF08 | Marek, Davis & Pierron 2017, “Sensitivity-based virtual fields for the non-linear virtual fields method”, DOI [10.1007/s00466-017-1411-6](https://doi.org/10.1007/s00466-017-1411-6) | 非线性材料/塑性场 | SBVF；线性硬化与 Voce 数值识别 | 是 | 是 | 视案例 | 未报告 | 可扩展 | 是 | 官方全文+图像（在线核验；本地 PDF/MinerU 待补） | 支持边界合力条件、参数激活、multi-start、噪声和虚场超参数审查；不把 SBVF 宣称为首次 |
...
| VF14 | Wittevrongel et al. 2015, “Evaluation of Methodologies for Compensation of Out of Plane Motions in a 2D Digital Image Correlation Setup”, DOI [10.1111/str.12146](https://doi.org/10.1111/str.12146) | 多次拉伸；2D DIC | 面外补偿 | 是 | 否 | 可能 | 未报告 | 未报告 | 未报告 | 官方页 | 刚体/面外预试验 |
...
| 同行评审/审稿全文+图像 | 15 | PA22–PA25、PA19A、PA28、BX05、BX16、BX18、VF08、VF25–VF28、DIC01 已通过官方开放全文或本地全文实际读取正文/图注或图表；其中 14 条已完成项目本地 PDF/MinerU 或 pypdf 绑定：P00、PA17、PA19A、PA22–PA25、PA28、BX05、BX16、BX18、BX19、VF25、DIC01；VF08 为在线官方全文核验 | 不能把不同材料或不同工艺批次的参数/模型移植到当前 PA12 |
| 公开会议全文+图像 | 1 | PA17 的 UGent 公开会议全文已由 pypdf 与 MinerU 实际读取正文、图注和提取图像；它用于方向/DIC/速率线索，不计入同行评审期刊篇数 | 不能把会议全文的材料结论升级为同行评审期刊证据 |
| arXiv 预印本全文+图像 | 1 | BX19 的 14 页正文与 28 个提取图像对象已由 MinerU 读取；可作双轴几何/参数抽取方法线索 | 不计同行评审论文数，不单独支撑材料本构或国际缺口 |
| 官方页（未完成正文/图像） | 38 | 作者/题名/期刊/DOI 入口或摘要级信息已核对，可支撑方向性判断 | 不等于正文和图像已经逐篇审计 |
...
本轮对本注册表的 89 个唯一 ID 做了记录级字段盘点，覆盖目标 `≥50`。这里的“状态清楚”指每条记录都明确标出证据等级和字段是否为 `是/否/未报告/可能/视案例`；它不把未报告字段升级为已证实事实。

### E.1 记录级状态映射

| 本表原状态 | 统一审计状态 | 数量 | 含义 |
|---|---|---:|---|
| `全文+图像`、`官方全文+图像`及在线官方全文 | 已核验 | 18 | 正文、图注或图表已实际读取；其中 P00 是项目内部全文，PA17 是公开会议全文，BX19 是 arXiv 预印本，同行评审/审稿条目为 15 篇；14 条已完成项目本地 PDF/MinerU 或 pypdf 绑定，VF08 为在线官方全文核验 |
| `官方页` | 摘要核验 | 37 | 官方期刊/出版社/DOI 页面已核对；未完成本项目登记所需的正文与图像审计 |
| `本地引用` | 仅线索 | 33 | 有本地题录或引用位置，但没有在本轮取得可直接复核的外部来源页 |
...
|---|---|---|
| 作者、年份、题名 | 86/89 行显式给出作者与年份；BX14、VF24、VF27、VF28 仍缺作者或年份；PA19A 的来源为审稿会议 PDF、无 DOI | 题名占位或作者缺失的记录不能进入投稿参考文献表 |
| 材料、实验 | 89/89 行有登记值 | 只有“已核验”子集的正文/图像证据可支撑实验细节；其余仍按摘要级或线索级使用 |
| 本构/模型、识别方法 | 89/89 行有登记值；可辨识性为“是”28、“部分”23、“未报告”32、“未重点报告”6 | “未报告/部分”不能写成已完成参数可辨识性验证 |
| DIC | 是 51；否 2；非核心 3；辅助 1；相关 1；全场 1；未报告 30 | 30 条未报告，不能当作无 DIC；但也不能当作使用了 DIC |
| VFM | 是 29；相关 5；否 55 | 相关不等于 VFM 识别已完成 |
| 双轴 | 是 27；组合 1；可能 14；视案例 7；未报告 10；否 30 | 只有 27 条明确写“是”；不能把可能/视案例并入双轴先例计数 |
| 速率 | 是 11；否 17；可能 9；视案例 5；间接 1；未报告 46 | 速率字段是当前最大未报告项之一，不能据此比较材料速率规律 |
| 各向异性 | 是 28；否 10；可能 20；可扩展 10；视案例 3；近似各向同性 1；未重点报告 1；未报告 16 | 可能/可扩展/视案例不是当前材料的已证实各向异性 |
| 主要创新、局限 | `LITERATURE_MAP.md` 核心矩阵规范化 15/89 行；数据库其余 74 行没有独立的创新/局限字段 | 不能把 15 条核心矩阵的判断外推为 89 篇逐篇创新审计 |
| 与本项目关系 | 89/89 行有“本项目用途” | 用途字段是项目决策，不是论文原文结论 |
| 真实来源链接 | 题录行有直接 URL 48/89；其中 44 个 DOI 通过元数据核验，VF18 失败；41/89 行没有直接外部链接 | 本地引用和项目内部全文仍需补作者/出版社/DOI 或机构全文入口 |

无直接外部链接的 ID：`P00, PA01–PA15, PA20–PA21, BX01–BX02, BX04, BX06–BX09, VF02–VF03, VF05–VF07, VF09, VF11, VF25–VF29, VF40`。其中 VF25–VF28 有本地正文解析记录，但尚未在题录行补入可追溯外部来源链接。
...
- VF14 的原登记题名曾误写为 “Improvement of DIC measurements using out-of-plane motion compensation”；已按 [Wiley 官方记录](https://onlinelibrary.wiley.com/doi/full/10.1111/str.12146) 更正为正式题名。`10.1111/str.12146` 与该记录匹配；本条仍只有官方页证据，未计入本地 PDF 正文/图像证据。
- `LITERATURE_BODY_IMAGE_EVIDENCE.md` 当前登记 PA22–PA25 与 VF08 的官方开放全文正文/图像核验；`MINERU_BODY_IMAGE_EVIDENCE.md` 登记 PA19A、BX16、BX18、VF25–VF28、DIC01、BX19 以及 PA22–PA25 的本地解析。DIC01 有 15 页正文和 63 个提取图像对象，BX19 有 14 页正文和 28 个提取图像对象，PA19A 有 10 页正文、11 个 PDF 图像对象且 MinerU 已提取图注/文字，PA22 有 18 页正文和 40 个提取图像文件，PA23 有 13 页正文和 20 个提取图像文件，PA24 有 10 页正文和 20 个提取图像文件，PA25 有 15 页正文和 43 个提取图像文件。VF08 的正文、公式、表格和图 7、9–17 已由 Springer/Europe PMC 在线全文核验，但本地 PDF/MinerU 仍待补；BX19 仍是 arXiv 预印本，不计同行评审全文；图像对象数量不等于独立实验结果图数量。
...
1. 优先补 PA17–PA19、PA26–PA28 的合法 PDF、正文和图像证据，确认 SLS PA12 的方向/速率/历史依赖证据；PA22–PA25 已完成本地 PDF/MinerU 绑定；
