...
已对真实 `XY_Y-09-0.1-02` 的 `Job.m2inp` 和 1,825 个逐帧 `.dat` 执行接入检查。`.dat` 可逐帧解析，但 `MatchIDReader.exe` 将 `Job.m2inp` 作为 Dataset 参数时没有返回 `IDataset`，退出码为 `-532462766`，未生成正式 CSV。真实候选输出见 [Phase 6 接入记录](../../wiki/02_实验与VFM/02_阶段计算/Phase6-真实MatchID数据接入-2026-09-18.md)。
