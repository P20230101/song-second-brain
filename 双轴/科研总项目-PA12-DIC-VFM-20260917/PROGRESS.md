...
- synthetic Gate 5 的 M0 正确模型 20-start LHS 已全部收敛（20/20、0 失败）；固定种子为 `20260917`，参数标准差为 `[1.28e-8, 4.49e-7]`，最好/中位/最差最大相对参数误差为 `1.4208e-5/1.4217e-5/1.4224e-5`。M1/M2 多初值仍在运行，结果不能外推到整个模型阶梯。
...
| 5 多初值 | synthetic M0 部分通过（20/20 LHS）；M1/M2 待完成 |
...
1. 完成 M1/M2 的 LHS 多初值矩阵，并按相同优化阈值记录所有收敛/失败实例；随后运行明确标注的 synthetic 噪声和模型形式误差情景。
