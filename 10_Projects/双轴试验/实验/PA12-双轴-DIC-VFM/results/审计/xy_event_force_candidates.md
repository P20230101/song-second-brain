...
- `load_onset`、`force_peak`、`force_drop` 是力侧事件投影到图像帧的候选；`file_last` 和 `visual_last_event` 是图像末端候选。对 `XY_XY-0.1-02`，峰值按帧 235–236 的窗口使用，帧 236 的四通道平均力最大。
- VFM 试跑先使用事件顺序完整且图像证据明确的试验；`XY-0.1-02` 的原始 MatchID `.dat` 已覆盖 0–258，破坏候选帧不再是“没有 DIC 输入”，但末帧 258 仅有 7 599/10 505 点，应只作破坏事件参考并按有效掩膜处理。
