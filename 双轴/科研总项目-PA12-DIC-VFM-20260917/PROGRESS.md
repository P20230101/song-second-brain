...
1. 使用生成 M2 多初值结果的完整 SciPy/joblib 运行环境复现失败起点；当前 SMApy 运行时的 SciPy–MKL LAPACK 工作区错误不能替代模型判断。只有解释/解决触限或正式将 M2 从首篇阶梯降级后，才运行其噪声和模型形式误差情景。
