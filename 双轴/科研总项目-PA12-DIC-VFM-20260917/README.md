...
"原始存在"不等于"VFM 可用"。当前已确认用户给出的中心厚度为 1.0 mm，S16 图像 manifest 覆盖 259 帧；原始 MatchID `.dat` 也覆盖 0–258 且全部可解析，主试样已导出 259 帧逐点标准化表，`DIC-xy_0.2` 则是 0–132 的独立简洁导出（其中 0–131 质量稳定、132 无效）。事件表没有有效共同触发事件，字段语义、最终有效掩膜和边界力绑定仍需验证。因此可以立即开展稳定帧候选 VFM，不能宣称已完成全程真实 VFM 识别。
