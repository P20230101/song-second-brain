...
这是从真实本地题录、已解析全文、Wiki 矩阵和官方 DOI/出版社入口整理的审计注册表，不是“已经逐篇读完 60 篇论文”的声明。`正文/图像证据` 是本表的硬状态：只有实际打开并读取正文、图注或提取图像的条目才标为 `全文+图像`；从另一篇论文参考文献读出的题录只能标为 `本地引用`。
...
| 同行评审全文+图像 | 5 | 已实际读取 VFM-GA、橡胶全场 VFM、FE-VFM/FEMU、有限体积场量和钢板十字形论文的正文/图注 | 不能把不同材料的参数或模型移植到 PA12 |
...
1. 优先补 PA17–PA19、PA22–PA27 的全文和图像，确认 SLS PA12 的方向/速率/历史依赖证据；
