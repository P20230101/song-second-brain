...
| 5 多初值 | synthetic M0、M1 通过（各 20/20 LHS、0 失败）；M2 待完成 |
...
1. 完成 M2 的 LHS 多初值矩阵，并按相同优化阈值记录所有收敛/失败实例；随后运行明确标注的 synthetic 噪声和模型形式误差情景。
