...
## 6. 84 条记录的字段审计结论（2026-09-17）

逐条字段盘点与 ID 清单见 [LITERATURE_DATABASE｜字段状态审计](LITERATURE_DATABASE.md#e-字段状态审计-2026-09-17)。本轮已将 84/84 条记录映射到 `已核验/摘要核验/仅线索/缺失`：10/39/34/1。这里的“已核验”只表示正文、图注或图表被实际读取，不表示 84 条都已完成全文审计。

当前最实质的缺口不是候选数量，而是证据粒度：主要创新/局限只在核心矩阵 15 条中规范化；42 条数据库记录没有直接外部来源链接；VF18 的原登记 DOI 未通过 Crossref/OpenAlex 元数据核验。故研究 Gap 仍只能写成“在明确工艺、批次和方向的 SLS PA12 中，比较整体曲线与 DIC 全场 + 边界力在参数可辨识性、误差传播和完全留出路径预测上的差异”，并标注为中等置信度候选缺口，不能写成 84 篇逐篇全文审计后确认的国际空白。
